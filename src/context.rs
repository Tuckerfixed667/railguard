...
pub fn generate_context(
    ...
    out.push_str(&format!("# Railyard Session Context: {}\n\n", session_id));
    ...
    if !snapshots.is_empty() {
        ...
        out.push_str(&format!(
            "```bash\n# Undo the last edit\nrailyard rollback --session {} --steps 1\n\n",
            session_id
        ));
        out.push_str(&format!(
            "# Undo the last N edits\nrailyard rollback --session {} --steps N\n\n",
            session_id
        ));
        out.push_str(&format!(
            "# Restore a specific file to its original state\nrailyard rollback --session {} --file <path>\n\n",
            session_id
        ));
        out.push_str(&format!(
            "# Restore everything to the session start\nrailyard rollback --session {} \n\n",
            session_id
        ));
